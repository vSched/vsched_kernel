...
{
	unsigned long task_util, util_min, util_max, best_cap, best_latency = 0;
	int cpu, best_cpu, best_latency_cpu = -1;
	struct cpumask *cpus;
	...
	for_each_cpu_wrap(cpu, cpus, target) {
		...
		unsigned long cpu_lat = cpu_rq(cpu)->max_latency;
		if (!available_idle_cpu(cpu) && !sched_idle_cpu(cpu))
			continue;
		if (util_fits_cpu(task_util, util_min, util_max, cpu)){
			if(best_latency > (cpu_lat)){
				best_latency = cpu_lat;
				best_latency_cpu = cpu;
			}
		}
		if (cpu_cap > best_cap) {
			best_cap = cpu_cap;
	...
	if(best_latency_cpu != -1){
		return best_latency_cpu;
	}
	return best_cpu;
}
...
select_task_rq_fair(struct task_struct *p, int prev_cpu, int wake_flags)
{
	...
	bpf_sched_cfs_latency_profile(new_cpu,0);
	rcu_read_unlock();
